"use strict";var onUnload=function(){event.returnValue=1},URL=window.location.hostname+window.location.pathname,URL_ARR=URL.split("/");window.addEventListener("dragover",function(){event.preventDefault()}),window.addEventListener("drop",function(){event.preventDefault()}),document.querySelector("body").addEventListener("click",function(a){var b=a.target;if(-1==b.className.search("menu")&&-1==b.className.search("status")&&"I"!=b.nodeName){var c=document.querySelectorAll("input[class='menu']");[].filter.call(c,function(a){a.checked=!1})}});
"use strict";function httpRequest(a,b,c,d,e){var f=5<arguments.length&&arguments[5]!==void 0?arguments[5]:null,g=new XMLHttpRequest;g.open(a,b,!0),g.onload=function(){if(200<=this.status&&400>this.status){var a=JSON.parse(this.response);d(a)}else e(this)},g.onprogress=function(){},g.onerror=function(){f()},g.send(c)}
...
'use strict';var num=document.querySelector('#status-num'),type=document.querySelector('#status-type'),place=document.querySelector('#status-place');2<URL_ARR.length?(document.querySelector('#type-'+URL_ARR[2]).selected=!0,document.querySelector('#type-num').selected=!0):document.querySelector('#status-type').selectedIndex=0;function goToList(){var a='';'#'===type.value&&(a='/'+place.innerHTML),a='/'+place.innerHTML+'/'+type.value+'/list',window.location.href=a}type.addEventListener('change',goToList),num.addEventListener('change',goToList);