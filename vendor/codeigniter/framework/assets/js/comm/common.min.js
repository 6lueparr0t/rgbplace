"use strict";let onUnload=function(){event.returnValue=1};var URL=window.location.hostname+window.location.pathname,URL_ARR=URL.split("/");window.addEventListener("dragover",function(){event.preventDefault()}),window.addEventListener("drop",function(){event.preventDefault()}),document.querySelector("body").addEventListener("click",function(a){let b=a.target;if(-1==b.className.search("menu")&&-1==b.className.search("status")&&"I"!=b.nodeName){let a=document.querySelectorAll("input[class='menu']");[].filter.call(a,function(a){a.checked=!1})}});
...
function httpRequest(a,b,c,d,e,f=null){let g=new XMLHttpRequest;g.open(a,b,!0),g.onload=function(){if(200<=this.status&&400>this.status)if(this.response){let a=JSON.parse(this.response);d(a)}else e(this)},g.onprogress=function(){},g.onerror=function(){f()},g.send(c)}