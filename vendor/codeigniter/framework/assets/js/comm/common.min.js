"use strict";var onUnload=function(){event.returnValue=1},URL=window.location.hostname+window.location.pathname,URL_ARRAY=URL.split("/");window.addEventListener("dragover",function(){event.preventDefault()}),window.addEventListener("drop",function(){event.preventDefault()}),document.querySelector("body").addEventListener("click",function(a){var b=a.target;if(-1==b.className.search("menu")&&-1==b.className.search("status")&&"I"!=b.nodeName){var c=document.querySelectorAll("input[class='menu']");[].filter.call(c,function(a){a.checked=!1})}});
...
'use strict';var place=document.querySelector('#status-place'),type=document.querySelector('#status-type'),num=document.querySelector('#status-num'),act=document.querySelector('#status-act');2<URL_ARRAY.length?(document.querySelector('#type-act-default').selected=!0,document.querySelector('#type-'+URL_ARRAY[2]).selected=!0,document.querySelector('#type-num').selected=!0):document.querySelector('#status-type').selectedIndex=0;function goToURL(){var a='';a='/'+place.innerHTML+'/'+type.value+'/'+this,'#'===type.value&&(a='/'+place.innerHTML),window.location.href=a}type.addEventListener('change',goToURL.bind('list')),num.addEventListener('change',goToURL.bind('list')),act.addEventListener('change',goToURL.bind(URL_ARRAY[3]));